...
func main() {
	...
	if err := os.MkdirAll(fmt.Sprintf("~/.local/ccc-lint/%s", version), 0o744); err != nil {
		fmt.Printf("Error: %+v\n", err)
		os.Exit(1)
	}

	execCommand("cp", fmt.Sprintf("%s/pkg/mod/github.com/cccteam/ccc/lint@%s/ccc-lint.so", gopath, version), fmt.Sprintf("~/.local/ccc-lint/%s/ccc-lint.so", version))
